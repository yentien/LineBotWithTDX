...
async function handleEvent(event) {
  const text = await prepareText(event);
  return client.replyMessage(event.replyToken, { type: 'text', text: text });
};

//detect keyword and prepare text to return
async function prepareText(event) {
  let replyToLine = "";
  if (event.type !== 'message' || event.message.type !== 'text') {
    ...
    replyToLine = exampleMessage();
  }
  else if (event.message.text.substring(0, 2) === "起站") {
    replyToLine = await sendTrainsSchedule(event.message.text);
  }
  else if (event.message.text.substring(0, 2) === "縣市") {
    replyToLine = sendStationOfCity(event.message.text);
  }
  else if (event.message.text.substring(0, 2) === "車次") {
    replyToLine = trainScheduleExample();
  }
  else if (event.message.text.substring(0, 2) === "車站") {
    replyToLine = stationExample();
  }
  return replyToLine;
}
...
const transformLineMessage = async (value) => {
  let arr = value.split("\n");
